...
# Load the dataset
try:
    df = pd.read_csv("/dados/sensores/dados_sensores.csv")
except FileNotFoundError:
    print("The file 'dados_sensores.csv' was not found. Please make sure it is in the correct directory.")
    exit()
...
if len(numerical_features) > 0:
    df[numerical_features].hist(bins=15, figsize=(15, 10), layout=(-1, 4), color='coral')
    ...
    print("No numerical features found to plot histograms (excluding potentially encoded target).")
...
if df[target_column].dtype == 'object' and len(numerical_features) > 0:
    for feature in numerical_features:
        if feature != target_column and feature != target_for_model :
            ...
            plt.show()
            print(f"Saved boxplot for {feature} to boxplot_{feature}_by_{target_column}.png")
elif len(numerical_features) == 0:
    print("Skipping box plots as no numerical features were found.")
else:
    print(f"Skipping box plots by target as the target variable '{target_column}' is not categorical or no numerical features.")
...
if len(numerical_features) > 0 :
    ...
    df_for_pairplot_data = df[cols_for_pairplot].copy() # Create a copy to avoid SettingWithCopyWarning
    ...
    elif df[target_for_model].nunique() < 10: # If numerical target has few unique values, can be used for hue
        target_for_hue = target_for_model
        df_for_pairplot_data[target_for_model] = df[target_for_model]

    if target_for_hue:
        sns.pairplot(df_for_pairplot_data, hue=target_for_hue, palette='husl', diag_kind='kde')
    else:
...
if X.isnull().sum().sum() > 0:
    print("Error: NaN values still present in features. Please check data cleaning steps.")
    exit()
...
print(f"Number of unique classes in target: {num_classes}. Using '{average_method}' averaging for recall and F1-score.")
...
for name, model in trained_models.items():
    ...
    results[name] = {"Accuracy": accuracy, "Recall": recall, "F1-score": f1, "Confusion Matrix": cm}
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                xticklabels=le.classes_ if 'le' in globals() and hasattr(le, 'classes_') and len(le.classes_) == num_classes else 'auto',
                yticklabels=le.classes_ if 'le' in globals() and hasattr(le, 'classes_') and len(le.classes_) == num_classes else 'auto')
